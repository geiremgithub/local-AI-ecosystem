...
SKRIPT_VERSJON = "v60-full-rag-logg-tilpasset"
...
def besvar_sporsmal(bruker_sporsmal):
    ...
    sok_resultat = db_samling.query(query_texts=[bruker_sporsmal], n_results=8)
    ...
    # Identifiser eventuelle nøkkelord fra anførselstegn eller deler av spørsmålet
    sitat_treff = re.findall(r'["\'«]([^"\'»]{2,})["\'»]', bruker_sporsmal)
    ord_i_sporsmal = [ord.lower() for ord in sitat_treff] if sitat_treff else [w.lower() for w in bruker_sporsmal.split() if len(w) > 3]
    ...
    for i in range(len(dokumenter)):
        ...
        # Sjekk om noen ord matcher i teksten for nøkkelord-indikator (1 eller 0)
        nøkkelord_match = 1 if any(o in dokumenter[i].lower() for o in ord_i_sporsmal) else 0

        # Formater smakebit slik at den matcher utseendet på bildet
        smakebit = dokumenter[i].strip().replace("\n", " ")
        if len(smakebit) > 55:
            smakebit = smakebit[:55] + "..."

        print(f"  [{i}] kilde={kilde} chunk={chunk_nr} avstand={avst:.4f} nøkkelord={nøkkelord_match} | \"{smakebit}\"")

        utdrag_tekst = f"Fil: {kilde} (Del {chunk_nr})\nInnhold:\n{dokumenter[i]}"
        if utdrag_tekst not in filtrerte_kilder:
            filtrerte_kilder.append(utdrag_tekst)

    antall_bit_analysert = min(5, len(filtrerte_kilder))
    forste_kilde = metadataer[0].get("kilde", "ukjent") if metadataer else "ukjent"
    print(f"AI-en analyserer {antall_bit_analysert} bit(er), best treff fra: {forste_kilde}...")

    generer_og_lagre_visuell_figur(bruker_sporsmal, dokumenter, metadataer, avstander, sporsmal_vektor, dok_vektorer)

    kombinert_kontekst = "\n\n".join(filtrerte_kilder[:antall_bit_analysert])

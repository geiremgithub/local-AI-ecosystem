...
SKRIPT_VERSJON = "v30-vektor-tall-visning"
...
def generer_og_lagre_visuell_figur(bruker_sporsmal, dokumenter, metadataer, avstander):
    ...
    try:
        ...
        plt.title("Vektorrom-visualisering av RAG-søk med tekstinnhold", fontsize=14, fontweight='bold')
...
def besvar_sporsmal(bruker_sporsmal):
    ...
    # Generer vektoren for spørsmålet
    sporsmal_vektor = embedding_modell([bruker_sporsmal])[0]
    # Vis de første 10 tallene i vektorenformat [x, x, x...]
    formaterte_tall_sporsmal = [round(float(v), 4) for v in sporsmal_vektor[:10]]
    print(f"2. Embedding (Spørsmål-vektor): Totalt {len(sporsmal_vektor)} dimensjoner.")
    print(f"   - Vektortall (første 10): {formaterte_tall_sporsmal} ...")
    ...
    for i in range(min(5, len(dokumenter))):
        ...
        chunk_nr = metadataer[i].get("chunk_nr", 0)
        avst = avstander[i]
        dok_vektor_tall = [round(float(v), 4) for v in dok_vektorer[i][:8]] # Viser de første 8 tallene
        print(f"     [{i}] Kilde: {kilde} (Chunk {chunk_nr}) | Avstand: {avst:.4f}")
        print(f"         Vektortall (første 8): {dok_vektor_tall} ...")

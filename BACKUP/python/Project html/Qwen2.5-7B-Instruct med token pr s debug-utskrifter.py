...
SKRIPT_VERSJON = "v24-terminal-RAG-innsyn"
...
def besvar_sporsmal(bruker_sporsmal):
    ...
    # 1. VISER TOKENISERING AV SPØRSMÅLET
    # Tokenizeren deler opp teksten i modellens ord- eller stavelser-IDer
    ...
    print(f"   - Tokens (ord/stavelser): {sporsmal_tokens}")
    print(f"   - Token ID-er: {sporsmal_token_ids}")

    # 2. VISER EMBEDDING (VEKTORISERING)
    # Kjører embedding-modellen manuelt på spørsmålet for å vise noen av tallverdiene
    vektorer = embedding_modell([bruker_sporsmal])
    forste_vektor_tall = vektorer[0][:5] # Viser de 5 første tallene i vektoren

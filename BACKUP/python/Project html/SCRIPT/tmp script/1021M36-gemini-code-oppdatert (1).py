...
def _del_lang_tekst(tekst, maks_lengde, overlapp):
    """
    Deler opp en lang tekstbit i mindre biter på setningsgrenser, med litt
    overlapp mellom bitene. Overlappen sikrer at fakta (datoer, tall, navn)
    ikke blir kuttet bort fra setningen som forklarer hva de gjelder.
    """
    ...
    for setning in setninger:
        ...
        if len(naavaerende) + len(setning) + 1 <= maks_lengde:
            ...
            # Start neste bit med en overlapp fra slutten av forrige bit,
            # slik at kontekst ikke går tapt over et chunk-skille.
...
def del_tekst_i_chunks(tekst, maks_lengde=800, overlapp=150):
    """
    Deler opp et dokument i chunks i tre trinn:
      1. Prøv å splitte på nummererte regler/punkter (f.eks. "1. Regel...").
      2. Hvis dokumentet ikke er en nummerert liste, splitt på avsnitt
         (tomme linjer).
      3. Hvis en enkelt bit (regel eller avsnitt) fortsatt er for lang,
         del den videre på setningsgrenser med overlapp, slik at ingen
         chunk blir så stor at embeddingen "utvannes" og enkeltfakta
         (som datoer eller klokkeslett) blir vanskelige å finne igjen
         ved søk.
    """
    tekst = tekst.strip()
    if not tekst:
    ...
    if len(nummererte_deler) > 1:
        ...
        # Trinn 2: avsnitt (splitt på en eller flere tomme linjer)
        avsnitt = re.split(r'\n\s*\n', tekst)
        avsnitt = [a.strip() for a in avsnitt if a.strip()]
        grunn_chunks = avsnitt if len(avsnitt) > 1 else [tekst]

    # Trinn 3: del videre opp chunks som er for lange
    endelige_chunks = []

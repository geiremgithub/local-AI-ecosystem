...
SKRIPT_VERSJON = "v52-robust-samtalehistorikk"
...
def sok_etter_ord_i_filer(nøkkelord):
    print(f"\n--- [Søker gjennom kildefiler etter '{nøkkelord}'] ---")
...
def besvar_sporsmal(bruker_sporsmal):
    ...
    for doc, meta in zip(alle_docs['documents'], alle_docs['metadatas']):
        if any(oi.lower() in doc.lower() for oi in ord_i_sporsmal):
            ...
            if utdrag not in filtrerte_kilder:
                ...
                if kilde_navn not in funnede_filer_hybrid:
                    print(f"[System] Søker gjennom kildefiler: fant treff i {kilde_navn}")
    ...
    meldinger = [
        {
            "role": "system",
            "content": (
                "Du er en streng dokumentassistent. Du har ABSOLUTT FORBUD mot å bruke generell eller ekstern kunnskap. "
                "Du skal svare UTELUKKENDE basert på det som står eksplisitt i de oppgitte kildene nedenfor.\n\n"
                "STRENGER REGLER FOR SVAR:\n"
                "1. Hvis en opplysning IKKE står ordrett eller direkte i kildeteksten, SKAL du skrive at informasjonen mangler i kildene.\n"
                "2. Det er strengt forbudt å fylle inn detaljer basert på generell ekstern kunnskap eller hva som er vanlig i den virkelige verden.\n"
                "3. Svar direkte og kortfattet. Ikke legg til høflighetsfraser eller forklaringer."
            )
        }
    ]

    # Inkluder tidligere samtalehistorikk slik at modellen husker konteksten
    meldinger.extend(SAMTALE_HISTORIKK)

    # Legg til det nye spørsmålet og de tilhørende kildene
    meldinger.append({
        "role": "user",
    ...
    # Lagre spørsmål og svar i historikken, og begrens til maks antall meldinger
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            if sp_lower.startswith("hvor finnes ordet") or sp_lower.startswith("finn alle kildefiler") or sp_lower.startswith("hvilke filer inneholder"):
                match = re.search(r'["\']([^"\']+)["\']', sporsmal)
                if match:
                    ordet = match.group(1)
                else:
                    ordet = sp_lower
                    for prefix in ["hvor finnes ordet", "finn alle kildefiler som har ordet", "finn alle kildefiler", "hvilke filer inneholder"]:
                        if ordet.startswith(prefix):
                            ordet = ordet[len(prefix):].strip()
                    ordet = ordet.strip("?.:\"'")

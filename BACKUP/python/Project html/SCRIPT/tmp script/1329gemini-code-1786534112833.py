...
SKRIPT_VERSJON = "v45-agentisk-sok-komplett-fix"
...
# --- AGENT-VERKTØY: Fleksibelt, robust eksakt tekstsøk på tvers av alle mapper ---
def søk_eksakt_tekst(sokefrase):
    """Skanner alle mapper etter en søkefrase uavhengig av store/små bokstaver og returnerer JSON.
    Henter ut både filnavn, sti og relevante avsnitt der frasen forekommer."""
    print(f"\n[Agent-Verktøy] Kjører fleksibelt søk etter: '{sokefrase}'")
    resultater = []
            ...
            for filnavn in filer:
                ...
                try:
                    with open(full_sti, "r", encoding="utf-8") as f:
                        innhold = f.read()
                        if mønster.search(innhold):
                            avsnitt = [avs.strip() for avs in innhold.split("\n\n") if mønster.search(avs)]
                            if not avsnitt:
                                avsnitt = [innhold[:300]]
                            resultater.append({
                                "filnavn": filnavn,
    ...
    meldinger = [
        {
            ...
            "content": (
                ...
                "Hvis verktøyresultatene viser flere filer, MÅ du liste opp alle filene, vise hva hver enkelt fil inneholder, "
                "og opprette en klar struktur eller punktliste som viser nøyaktig hva som varierer mellom filene.\n\n"

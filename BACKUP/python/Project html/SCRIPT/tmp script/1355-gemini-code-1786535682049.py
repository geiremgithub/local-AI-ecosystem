...
SKRIPT_VERSJON = "v46-agentisk-sok-streng-kontroll"
...
generasjons_konfig = GenerationConfig(
    max_new_tokens=400,
    do_sample=False,  # Satt til False for å minimere hallusinering og sikre deterministiske svar
    temperature=0.0,
)

# --- AGENT-VERKTØY: Nøyaktig tekstsøk som henter faktisk filinnhold ---
                ...
                try:
                    with open(full_sti, "r", encoding="utf-8") as f:
                        ...
                        if mønster.search(innhold):
                            # Hent ut eksakte linjer/avsnitt som matcher
        ...
        try:
            ...
            for res in parset_resultat:
                ...
                for utd in res['utdrag']:
                    formatert_agent_tekst += f"linje: {utd}\n"
                formatert_agent_tekst += "\n"
        except Exception:
            formatert_agent_tekst = agent_resultat_json

        agent_ekstra_info = f"\n\n----- VERKTØYETS FAKTISKE FUNTREFF (BRUK KUN DISSE DATAENE) -----\n{formatert_agent_tekst}"
    ...
    meldinger = [
        {
            ...
            "content": (
                ...
                "Du skal IKKE finne på, gjette eller anta likheter mellom filer som ikke støttes 100% av verktøyresultatene. "
                "Hvis en fil inneholder en linje og en annen fil ikke gjør det, skal dette komme tydelig fram i forskjellen. "
                "Ikke overfør tekst fra én fil til en annen.\n\n"
                f"----- RAG-KONTEKST -----\n{kombinert_kontekst}"
                f"{agent_ekstra_info}"
            )
        },

...
SKRIPT_VERSJON = "v22-terminal-Qwen2.5-3B-med-debug"
...
def besvar_sporsmal(bruker_sporsmal):
    ...
    # Pakker ut data for debug-visning
    ...
    print("Debug - hentede biter (sortert etter avstand):")
    for i in range(len(dokumenter)):
        kilde = metadataer[i].get("kilde", "ukjent")
        chunk_nr = metadataer[i].get("chunk_nr", 0)
        avst = avstander[i]
        tekst_snutt = dokumenter[i].replace("\n", " ")[:50]
        
        # Sjekker enkelt etter nøkkelord i spørsmålet
        nokkelord_treff = 1 if any(ord in bruker_sporsmal.lower() for ord in kilde.lower().split("_")) else 0
        print(f"  [{i}] kilde={kilde} chunk={chunk_nr} avstand={avst:.4f} nøkkelord={nokkelord_treff} | \"{tekst_snutt}...\"")

    kombinert_kontekst = "\n\n".join(dokumenter[:5]) # Bruker de 5 beste bitene til AI-en
    print(f"AI-en analyserer 5 bit(er), best treff fra: {metadataer[0].get('kilde', 'ukjent')}...")
    ...
    # Viser kilde og svar slik du ønsket

...
SKRIPT_VERSJON = "script9-mcp-flask"
...
app = Flask(__name__)
mcp = FastMCP("DokumentAssistentMCP")
...
@mcp.tool()
def mcp_besvar_sporsmal(bruker_sporsmal: str) -> str:
    """MCP verktøy for å besvare spørsmål basert på RAG-pipelinen."""
    ...
    if not sok_resultat["documents"] or not sok_resultat["documents"][0]:
        return "Fant ingen relevante dokumenter."
    ...
    if ord_i_sporsmal:
        ...
        print("\n--- [SVAR: EKSAKT ORDSØK] ---")
        if funnede_lokasjoner:
            svar_tekst = f"Ordet '{', '.join(ord_i_sporsmal)}' ble funnet i følgende filer og lokasjoner:\n"
            for kilde, chunk_nr in sorted(funnede_lokasjoner):
                svar_tekst += f"  - {kilde} (Del {chunk_nr})\n"
        else:
            svar_tekst = f"Ordet '{', '.join(ord_i_sporsmal)}' ble ikke funnet i noen av kildefilene."
        print(svar_tekst)
        return svar_tekst.strip()
...
def hovudside():
    ...
    <head>
        <meta charset="UTF-8">
        <title>Dokumentoppslag</title>
        <style>
            body { font-family: Arial, sans-serif; background-color: #f4f6f9; margin: 0; padding: 40px; color: #333; }
            .container { max-width: 800px; margin: auto; background: white; padding: 30px; border-radius: 8px; box-shadow: 0 4px 10px rgba(0,0,0,0.05); }
            h1 { margin-top: 0; color: #111; }
            p.sub { color: #666; margin-bottom: 25px; }
            .input-group { display: flex; gap: 10px; }
            input[type="text"] { flex: 1; padding: 12px 15px; font-size: 16px; border: 1px solid #ccc; border-radius: 4px; outline: none; }
            input[type="text"]:focus { border-color: #0056b3; }
            button { padding: 12px 25px; font-size: 16px; background-color: #1e293b; color: white; border: none; border-radius: 4px; cursor: pointer; }
            button:hover { background-color: #334155; }
            #svar-boks { margin-top: 25px; padding: 20px; background: #f8fafc; border-left: 4px solid #1e293b; border-radius: 4px; display: none; white-space: pre-wrap; line-height: 1.5; }
        </style>
    </head>
    <body>
        <div class="container">
            <h1>Dokumentoppslag</h1>
            <p class="sub">Spør mot dokumentene i mappen — svarene bygger kun på det som faktisk står der</p>
            
            <div class="input-group">
                <input type="text" id="sporsmalInput" placeholder="Skriv spørsmålet ditt..." onkeydown="if(event.key === 'Enter') sendSporsmal()">
                <button onclick="sendSporsmal()">Spør</button>
            </div>
            
            <div id="svar-boks"></div>
        </div>

        <script>
            async function sendSporsmal() {
                const input = document.getElementById('sporsmalInput');
                const svarBoks = document.getElementById('svar-boks');
                const sporsmal = input.value.trim();
                
                if (!sporsmal) return;
                
                svarBoks.style.display = 'block';
                svarBoks.innerText = "Tenker... (analyserer dokumenter og genererer svar)";
                
                try {
                    const respons = await fetch('/api/sporsmal', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ sporsmal: sporsmal })
                    });
                    
                    const data = await respons.json();
                    if (data.svar) {
                        svarBoks.innerText = data.svar;
                    } else {
                        svarBoks.innerText = "Beklager, fikk ikke noe svar fra serveren.";
                    }
                } catch (err) {
                    svarBoks.innerText = "En feil oppstod under kommunikasjon med serveren.";
                }
            }
        </script>
    </body>
...
def flask_api_sporsmal():
    data = request.get_json()
    if not data or "sporsmal" not in data:
        return jsonify({"feil": "Mangler 'sporsmal' i JSON-payload"}), 400
    
    sporsmal = data["sporsmal"]
    svar = mcp_besvar_sporsmal(sporsmal)
    return jsonify({"sporsmal": sporsmal, "svar": svar})
...
if __name__ == "__main__":
    ...
    print("\n[System] Starter Flask server på port 5000...")
    flask_trad = Thread(target=lambda: app.run(host="0.0.0.0", port=5000, debug=False, use_reloader=False), daemon=True)
    flask_trad.start()

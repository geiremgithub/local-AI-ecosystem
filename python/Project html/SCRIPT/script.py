...
SKRIPT_VERSJON = "v59-full-rag-logg-alltid-synlig"
...
def besvar_sporsmal(bruker_sporsmal):
    ...
    def _normaliser_for_sok(tekst):
        # Fjerner vanlig tegnsetting og normaliserer mellomrom, slik at f.eks.
        # søkeordet "17 august" også matcher kildetekst som skriver "17. august".
        # Dette er en generell normalisering - ikke spesifikk for noe bestemt ord.
        tekst = tekst.lower()
        tekst = re.sub(r'[.,;:!?"\']', ' ', tekst)
        tekst = re.sub(r'\s+', ' ', tekst).strip()
        return tekst

    ord_i_sporsmal_normalisert = [_normaliser_for_sok(ord) for ord in ord_i_sporsmal]
    ...
    for doc, meta in zip(alle_docs['documents'], alle_docs['metadatas']):
        doc_normalisert = _normaliser_for_sok(doc)
        if any(oi in doc_normalisert for oi in ord_i_sporsmal_normalisert):
    ...
    if ord_i_sporsmal:
        ...
        # Et søk med ord i anførselstegn er et eksakt tekstoppslag, ikke et spørsmål
        # som krever tolkning. Svaret er allerede fullstendig og korrekt kjent fra
        # selve søket, så vi går ikke via språkmodellen her - det unngår at en liten
        # modell mistolker eller finner på noe rundt et svar vi allerede har eksakt.
        print("\n--- [SVAR: EKSAKT ORDSØK] ---")
        if funnede_lokasjoner:

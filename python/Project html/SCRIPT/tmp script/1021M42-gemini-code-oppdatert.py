...
def besvar_sporsmal(bruker_sporsmal):
    ...
    # Prøv først å finne konkrete søkeord i anførselstegn, f.eks. "skolestart".
    # Da er det brukeren selv som har definert hva som skal søkes etter,
    # i stedet for at ethvert ord >3 tegn i hele spørsmålet blir brukt som søkeord.
    sitat_treff = re.findall(r'["\'«]([^"\'»]{2,})["\'»]', bruker_sporsmal)

    STOPPORD = {
        "finn", "finner", "finnes", "ordet", "ordene", "vis", "hvor", "hvordan",
        "disse", "filene", "filer", "kildene", "kildefilene", "heter", "dersom",
        "ikke", "noen", "skriv", "håpløs", "at", "det", "denne", "dette", "hvilke"
    }
    ...
    if sitat_treff:
        ...
        ord_i_sporsmal = [
            word.lower().strip('?.,!:;"\'')
            for word in bruker_sporsmal.split()
            if len(word.strip('?.,!:;"\'')) > 3
            and word.lower().strip('?.,!:;"\'') not in STOPPORD
        ]
    ...
    for doc, meta in zip(alle_docs['documents'], alle_docs['metadatas']):
        if any(oi in doc.lower() for oi in ord_i_sporsmal):
            ...
            utdrag = f"Fil: {kilde_navn} (Del {meta['chunk_nr']})\nInnhold:\n{doc}"
    ...
    if ord_i_sporsmal:
        if funnede_filer_hybrid:
            print(f"[System] Nøkkelord ('{', '.join(ord_i_sporsmal)}') funnet i: {', '.join(sorted(funnede_filer_hybrid))}")
        else:

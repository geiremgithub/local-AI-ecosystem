...
def behandle_sporsmal(sporsmalstekst: str):
    ...
    # Formaterer til rene float-verdier slik at np.float32-teksten forsvinner
    vektor_utdrag_sporsmal = [float(round(x, 4)) for x in sporsmal_vektor[:6]]
    print(f"   - Spørsmålet ble vektorisert til {len(sporsmal_vektor)} dimensjoner.")
    print(f"   - Vektortall (utdrag): {vektor_utdrag_sporsmal} ...\n")
    
    # 3. Databasesøk og vektorer
    ...
    for i, idx in enumerate(sortert_indeks):
        ...
        dist = avstander[idx]
        vektor_utdrag_db = [float(round(x, 4)) for x in db_vektorer[idx][:6]]
        print(f"   [{i+1}] Kilde: {treff['kilde']} (Avstand: {dist:.4f})")
        print(f"       Tekst: \"{treff['tekst']}\"")
        print(f"       Vektortall: {vektor_utdrag_db} ...")
    print()

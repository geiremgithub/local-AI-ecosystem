...
SKRIPT_VERSJON = "v44-eksakt-ordtelling-og-kildedeling"
...
def besvar_sporsmal(bruker_sporsmal):
    ...
    formaterte_tall_sporsmal = [float(round(float(v), 4)) for v in sporsmal_vektor[:8]]
    ...
    # 3. Databasesøk og nøyaktig telling av søkeord på tvers av filer
    print("3. Databasesøk...")
    sok_resultat = db_samling.query(query_texts=[bruker_sporsmal], n_results=20)
    ...
    # Ekstra tellermekanisme for å sjekke forekomst av spesifikke ord (f.eks. "skolestart")
    # Vi henter ut alle unike dokumenter/filer som finnes i databasen for å gjøre en fullstendig opptelling
    alle_lagrede_data = db_samling.get()
    fil_forekomster = {}
    
    # Let etter nøkkelord i spørsmålet (f.eks. fjerner vanlige stoppord eller sjekker hele termer)
    sok_ord = bruker_sporsmal.lower().split()
    nokkelord = "skolestart" if "skolestart" in bruker_sporsmal.lower() else (sok_ord[-1] if sok_ord else "")

    if "tekster" in alle_lagrede_data or "documents" in alle_lagrede_data:
        lagrede_tekster = alle_lagrede_data.get("documents", [])
        lagrede_meta = alle_lagrede_data.get("metadatas", [])
        
        for t, m in zip(lagrede_tekster, lagrede_meta):
            kildenavn = m.get("kilde", "ukjent")
            if kildenavn not in fil_forekomster:
                fil_forekomster[kildenavn] = {"totalt_antall": 0, teksters_innhold: []}
            
            # Teller forekomster case-insensitivt
            antall_i_chunk = t.lower().count("skolestart")
            if antall_i_chunk > 0:
                if kildenavn not in fil_forekomster:
                    fil_forekomster[kildenavn] = 0
                fil_forekomster[kildenavn] += antall_i_chunk

    print("   - Analyse av søkeordforekomster i filene:")
    for fil, antall in fil_forekomster.items():
        if antall > 0:
            print(f"     * Fil '{fil}' inneholder ordet 'skolestart' {antall} gang(er).")
    ...
    for i in range(min(5, len(dokumenter))):
        ...
        chunk_nr = metadataer[i].get("chunk_nr", 0)
        avst = avstander[i]
        dok_vektor_tall = [float(round(float(v), 4)) for v in dok_vektorer[i][:8]]
        print(f"     [{i}] Kilde: {kilde} (Chunk {chunk_nr}) | Avstand: {avst:.4f}")
        print(f"         Vektortall: {dok_vektor_tall} ...")

    # 4. Vektorrom-visualisering
    generer_og_lagre_visuell_figur(bruker_sporsmal, dokumenter, metadataer, avstander, sporsmal_vektor, dok_vektorer)

    # Bygger en eksplisitt oversikt over hvilke filer som inneholder ordet, slik at LLM-en får fasiten direkte i systemmeldingen
    oppsummert_forekomst_tekst = ", ".join([f"'{fil}' ({antall} ganger)" for fil, antall in fil_forekomster.items() if antall > 0])
    kombinert_kontekst = "\n\n".join(dokumenter[:5])
    ...
    meldinger = [
        {
            ...
            "content": (
                ...
                "Du har nøyaktig oversikt over hvilke filer som inneholder søkeordet fra databasen. "
                f"Følgende fil- og ordanalyse gjelder for søkeordet: {oppsummert_forekomst_tekst}. "
                "Oppgi alltid hvilke dokumenter som inneholder ordet, nøyaktig hvor mange ganger det forekommer i hvert dokument, "
                "og forklar forskjellene mellom dokumentene basert på innholdet.\n\n"
                f"----- DOKUMENTINNHOLD -----\n{kombinert_kontekst}"
            )
        },
    ]
    ...
    print("4. LLM-Generering: Sender oppdatert prompt med nøyaktig filstatistikk til Qwen-modellen...")
    start_tid = time.time()

...
SKRIPT_VERSJON = "v46-robust-ordtelling-og-kontekst"
...
def besvar_sporsmal(bruker_sporsmal):
    ...
    # Identifiser søkeord (f.eks. "skolestart") dynamisk fra spørsmålet
    sok_term = "skolestart" if "skolestart" in bruker_sporsmal.lower() else ""
    if not sok_term:
        ord_liste = [o.strip("?.,!;:") for o in bruker_sporsmal.lower().split()]
        if ord_liste:
            sok_term = ord_liste[-1]

    # Nøyaktig telling av søkeordet direkte fra råteksten i databasen
    alle_lagrede_data = db_samling.get()
    fil_forekomster = {}
    fil_innhold_samling = {}

    if "documents" in alle_lagrede_data:
        ...
        for t, m in zip(lagrede_tekster, lagrede_meta):
            ...
            if kildenavn not in fil_forekomster:
                ...
                fil_innhold_samling[kildenavn] = []
            
            fil_innhold_samling[kildenavn].append(t)
            if sok_term:
                antall = t.lower().count(sok_term)
                fil_forekomster[kildenavn] += antall

    print(f"   - Nøyaktig søkeordanalyse for '{sok_term}':")
    for fil, antall in fil_forekomster.items():
        print(f"     * Fil '{fil}': {antall} forekomst(er)")
    ...
    # Bygg komplett og direkte kontekst fra filene slik at modellen ikke hallusinerer eller gjetter
    fullstendig_fil_kontekst = ""
    for kilde, c_liste in fil_innhold_samling.items():
        fullstendig_fil_kontekst += f"\n--- FIL: {kilde} ---\n" + "\n".join(c_liste) + "\n"

    statistikk_tekst = ", ".join([f"'{fil}' har {antall} treff på '{sok_term}'" for fil, antall in fil_forekomster.items()])

    meldinger = [
        {
            "role": "system", 
            "content": (
                "Du er en presis og sannferdig norsk assistent. Du skal basere svar utelukkende på filinnholdet under. "
                f"Statistikk over søkeordet '{sok_term}': {statistikk_tekst}. "
                "Ikke påstå at filer mangler eller ikke inneholder ordet hvis filinnholdet viser det. "
                "Vis nøyaktig hvilke filer som inneholder ordet, hvor mange ganger det forekommer, og forklar forskjellene mellom dokumentene.\n\n"
                f"----- FULLSTENDIG DOKUMENTINNHOLD -----\n{fullstendig_fil_kontekst}"
            )
        },
    ]
    ...
    print("4. LLM-Generering: Sender direkte filinnhold og nøyaktig statistikk til Qwen-modellen...")
    start_tid = time.time()

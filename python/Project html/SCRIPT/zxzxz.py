...
SKRIPT_VERSJON = "v21-terminal-Qwen2.5-3B-streng-systemprompt"
...
generasjons_konfig = GenerationConfig(
    max_new_tokens=200,
    do_sample=True,
    temperature=0.3, # Senket litt for mer presise og mindre «kreative» svar
)
...
def besvar_sporsmal(bruker_sporsmal):
    ...
    # Strengere systemmelding for å unngå fylltekst
    meldinger = [
        {
            "role": "system", 
            "content": (
                "Du er en presis assistent. Svar KUN direkte på det brukeren spør om. "
                "Ikke ta med ekstra informasjon, bakgrunnsdetaljer eller uoppfordrede fakta fra teksten. "
                "Svaret skal være kort og poengtert.\n\n"
                f"----- DOKUMENTINNHOLD -----\n{kombinert_kontekst}"
            )
        },
    ]
